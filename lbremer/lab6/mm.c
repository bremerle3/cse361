...
//handle the case of the input pointer being null by calling mm_malloc
  ...
  /*
  if(asize > prevSize){
  extraSize = asize - prevSize;
  nextSize = GET_SIZE(HRDP(NEXT_FREE(ptr)));
  prevSize = GET_SIZE(HRDP(PREV_FREE(ptr))));
  if(nextSize >= asize - prevSize + DSIZE){
  */
